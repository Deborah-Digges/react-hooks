...
- `useEffect(function)` is a hook that runs the code in `function` when a
  component has just been rendered (and every time it is re-rendered).
- `useEffect(function)`: `function` is executed every time the component is
  rendered, wheras if we want to only run it when a certain piece of state
  changes, we can use the "dependency array" for the effect:
  `useEffect(function, [stateVar])` which will run `function` only when
  `stateVar` changes.
- **Lazy state initialization** - `useState(initialValue)` can be expensive if
  `initialValue` is the result of an expensive computation. That computation
  will be made every time the component is rendered, although it is only needed
  _once_ when the state is initialized. A function which returns the initial
  state `useState(() => initialValue)` will be run only _once_ when the state is
  initialized and will avoid repeated computations when they are not needed.
- If a state variable needs to be updated based on it's previous value, it is
  safer to use a callback function to `setVar`, for example
  `setVar(prevValue => previousValue + 1)`.
- Common logic for state management and effects can be encapsulated into a
  "custom hook".
