...
- Some state should be **managed** and some state should be **derived**
- Having this distinction between derived and managed state helps prevent
  complex logic where we are trying to keep multiple state variables in sync.
  Derived variables are anyway automatically recomputed on a re-render.
