...
- When two siblings need the same piece of state, instead of copying the state
  between the two components, we "lift" the shared state into the lowest common
  ancestor
- This provides a single source of truth for the shared state and ensures that
  the sibling components are in sync. This reduces the surface area for bugs as
  a single component is responsible for changing shared state.
- Derived variables should not be stored in state, but rather, should be
  computed
- Conversely, when state is defined in a parent component, but is only needed by
  one of the children, we can colocate the state into only the child that needs
  it
