...
### Digging into the `useEffect` hook

- The `useEffect` hook cannot return anything apart from a cleanup function.
- If you would like to achieve any logic that you would typically put in the
  `componentDidUnmount` lifecycle method, return a cleanup function
  `() => { cleanupLogic() }` from `useEffect` that performs this same logic
- Since `useEffect` allows us to return only the cleanup function, we cannot use
  an `async` function in `useEffect` since this effectively returns a promise.
  The way to call an async function inside `useEffect` would be to use something
  like this:
...
- It would be simpler to extract this async logic into a utility and perform a
  `.then` on it
...
- The above two statements are slightly different semantically:

1. `.catch` will handle errors in both `doSomething` as well as `processData`
2. The second will only catch errors in `doSomething`

### State Machine

- Instead of having complex logic to determine what state your UI is in, it's
  best practice to maintain a finite set of states and clearly indicate which
  state is linked to which UI screen.
- This can also be achieved by using a library for building a state machine like
  `xstate`
- Multiple calls to `setState` in sequence could lead to renders between them
  resulting in flickers or errors. It would be best to avoid this and try to
  have a single state object that is updated by a single `setState` call

### Error Boundaries

- Before React 16, if an unhandled error occured during the rendering of a React
  component, it caused the React tree to be in an inconsistent state
- Error Boundaries were introduced in React 16 to make it easier to handle such
  errors
- An error boundary is a class component with a few methods:
  `getDerivedStateFromError(error)`, `componentDidCatch`, and `render`
- This error boundary component wraps any children which we wish to protect from
  unhandled errors
- The `getDerivedStateFromError` method sets some state if an error has occured.
- If an error has occured, the `render` method displays an appropriate error
  state screen, otherwise it just renders `props.children`
- However, if an error occurs and then we wish to "reset" the error boundary, we
  will effectively have to mount and unmount it in order to do this. We can
  achieve this using a `key` prop which contains the key in response to whose
  changes the error boundary should update (Ex. A new input is entered)
- Instead of having to maintain our own error boundary, we can use third party
  libraries like `react-error-boundary`
- In this library, it's possible to provide a `FallBackComponent`, an `onReset`,
  and `resetKeys` (whose function is to reset the error boundary in response to
  a change in the value of any of these values).
