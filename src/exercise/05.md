...
- JSX returned by the `render` method is syntactic sugar for
  `React.createElement`. None of these elements are actual DOM elements. Only
  when `ReactDOM.render` is called at a later point are DOM nodes actually
  created
- refs are useful when we need to manipulate DOM nodes directly as is sometimes
  the case with some libraries (ex. animation libraries)
- A ref is an object which is constant over the mounting/unmounting of a
  component. It's `current` property points to the DOM node that is rendered for
  particular React element at that point in time.
- Using refs approximately follows the following pattern:

1. Create a ref: `const myRef = React.useRef()`
2. Pass this ref to the React element whose DOM node who wish to have a
   reference to: `<div ref={myRef}>`
3. Access the react element's underlying DOM node using `myRef.current` once the
   React element has been mounted (Rendered). This is done in the `useEffect`
   callback in functional components and in the `componentDidMount` method in
   class components
4. To perform any cleanup related to this DOM node, return a cleanup function
   from `useEffect` in functional components. In class components, cleanup logic
   can be placed in the `componentDidUnmount` method.
